...
This cleanup branch intentionally **fails closed** while a verified, redistributable encoder is unavailable.

No guessed or experimental UFR II LT/SFP byte stream is generated or sent to the physical printer.
...
The research also identified proprietary Canon runtime components used by the Linux driver. Those components are **not bundled with this Android project**. The Canon SLIM payloads previously present in the development tree have been removed from this branch's working tree and build path.

Reverse-engineering observations are kept separate from the shipped implementation. They are not treated as a substitute for an independently licensed implementation or a clearly redistributable specification.
...
## Cleanup branch purpose

`cleanup/remove-proprietary-encoder` exists to separate the safe, fail-closed implementation from earlier development work that contained Canon proprietary payloads. Normal file deletion does not erase those payloads from Git history; a history rewrite is therefore required before this branch can become the clean distributable project history.
